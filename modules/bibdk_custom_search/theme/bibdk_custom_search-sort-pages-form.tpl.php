...
drupal_add_tabledrag('pages', 'order', 'sibling', 'element-weight');

?>
<table id="pages" class="sticky-enabled">
  <thead>
    <tr>
    ...
    <?php foreach (element_children($form) as $key => $element):
      $data = $form[$element]; ?>
        <tr class="draggable <?php print $key % 2 == 0 ? 'odd' : 'even'; ?>">
          ...
          <td><?php if ($data['page_title']['#default_value']==='') { print drupal_render($data['page_title']); } ?></td>

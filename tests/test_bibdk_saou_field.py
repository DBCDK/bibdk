...
class TestSaouField(helpers.BibdkUnitTestCase):
    ...
    def test_saou_walkin_access(self):
        ...
        browser.find_element_by_xpath("//div[contains(.,'"+text+"')]")
    ...
    def test_not_logged_in(self):
        ...
        path = self.base_url + "search/work/rec.id=" + self.pid1
        browser.get(path)
        element = browser.find_element_by_id("selid-150008academicebr10655298")
        element.click()
    ...
    def text_is_present(self, element, text):
        try:
            self.browser.find_element_by_xpath("//div[contains(.,'"+text+"')]")
